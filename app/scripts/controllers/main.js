...
app.controller('MainCtrl', function ($scope, $sce, $rootScope, $timeout, $http, $interval, $filter, $anchorScroll, $location, $state, $uibModal, ClientFact, LogFact, Upload, NotifyingService /*FileUploader*/ ) {
    ...
    $scope.kibanaURL_inRange = "http://" + window.location.hostname + ":5601/app/kibana#/dashboard/instance-in-range-view?embed=true&_g=(refreshInterval:(display:Off,pause:!f,value:0),time:(from:now-15m,mode:quick,to:now))&_a=(filters:!(),options:(darkTheme:!f),panels:!((col:1,id:instances-in-range,panelIndex:2,row:1,size_x:12,size_y:6,type:visualization)),query:(query_string:(analyze_wildcard:!t,query:'*')),title:instance-in-range-view,uiState:(P-2:(spy:(mode:(fill:!f,name:!n)))))";
    
    $scope.kibanaURL_notInRange = "http://" +window.location.hostname + ":5601/app/kibana#/visualize/edit/instances-not-in-range?embed=true&_g=(refreshInterval:(display:Off,pause:!f,value:0),time:(from:now-15m,mode:quick,to:now))&_a=(filters:!(),linked:!f,query:(bool:(must:(term:(in_range:!f)),must_not:(exists:(field:end_time)))),uiState:(),vis:(aggs:!((id:'1',params:(),schema:metric,type:count),(id:'2',params:(autoPrecision:!t,field:location,mapCenter:!(13.239945499286312,32.34375),mapZoom:2,precision:2),schema:segment,type:geohash_grid)),listeners:(),params:(addTooltip:!t,heatBlur:15,heatMaxZoom:16,heatMinOpacity:0.1,heatNormalizeData:!t,heatRadius:25,isDesaturated:!f,mapType:'Scaled%20Circle%20Markers',wms:(enabled:!t,options:(attribution:'Maps%20provided%20by%20USGS',format:image%2Fpng,layers:'0',styles:'',transparent:!t,version:'1.3.0'),url:'https:%2F%2Fbasemap.nationalmap.gov%2Farcgis%2Fservices%2FUSGSTopo%2FMapServer%2FWMSServer')),title:instances-not-in-range,type:tile_map))";
